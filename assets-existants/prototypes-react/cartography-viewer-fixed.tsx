...
const calculateVisiblePoints = (node, currentDate) => {
  ...
  // Sum up points from all traces up to current date
...
const GitHubCalendar = ({ traces, currentDate, onDateClick, selectedYear }) => {
  ...
  adjustedStart.setDate(startDate.getDate() - (startDay === 0 ? 6 : startDay - 1));
  ...
  const activityMap = useMemo(() => {
    ...
    traces.forEach(trace => {
      ...
      if (!map[date]) {
        map[date] = { count: 0, points: 0, traces: [] };
      }
      map[date].count++;
      map[date].points += trace.points;
      map[date].traces.push(trace);
    });
    return map;
  }, [traces]);
  ...
  while (weekCount < 52 || currentWeek.length > 0) {
    const dateStr = currentDateIter.toISOString().split('T')[0];
    const activity = activityMap[dateStr] || { count: 0, points: 0, traces: [] };
    ...
    if (weekCount >= 52 && currentDateIter > endDate) break;
  }
  ...
  const getIntensityColor = (activity) => {
    ...
    if (activity.count <= 4) return '#30a14e';
    return '#216e39';
  };
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-700">Activité d'apprentissage</h3>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          {[0, 1, 2, 3, 4].map(level => (
            <div
              ...
              style={{ backgroundColor: getIntensityColor({ count: level }) }}
            />
          ))}
          <span>Plus</span>
        </div>
      ...
      <div className="relative">
        {/* Day labels */}
        <div className="absolute -left-7 top-5 text-xs text-gray-500">
          {days.map((day, i) => (
            <div key={i} className="h-[13px] leading-[13px]" style={{ visibility: i % 2 === 0 ? 'visible' : 'hidden' }}>
              {day}
            </div>
          ))}
        </div>
        ...
        {/* Month labels */}
        <div className="flex text-xs text-gray-500 mb-1 ml-2">
          {months.map((month, i) => (
            <div key={i} style={{ width: `${100/12}%` }} className="text-center">
              {month}
            </div>
          ))}
        </div>
              ...
              {week.map((day, dayIndex) => (
                <div
                  ...
                  className={`w-[11px] h-[11px] rounded-sm cursor-pointer hover:ring-1 hover:ring-gray-400 ${
                    day.dateStr === currentDateStr ? 'ring-2 ring-blue-500' : ''
                  }`}
                  style={{
                    backgroundColor: day.isInYear ? getIntensityColor(day.activity) : '#f0f0f0',
                    opacity: day.isInYear ? 1 : 0.3
                  }}
                  title={`${day.date.toLocaleDateString('fr-FR')}: ${day.activity.count} activité(s), ${day.activity.points} points`}
...
const TreeNode = ({ node, level = 0, onSelect, selectedId, currentView, hoveredId, expandedNodes, onToggle, currentDate, showTraces, onToggleTraces }) => {
  ...
  // Calculate visible points (including children)
  const visiblePoints = useMemo(() => {
    return calculateVisiblePoints(node, currentDate);
  }, [node, currentDate]);
  ...
  // Don't show group if it has no visible traces in all its descendants
  if (!node.isLeaf && visiblePoints === 0) {
    return null;
  }
      ...
      >
        ...
        {visiblePoints > 0 && (
          <span 
            ...
            title={`${visiblePoints} points d'expérience`}
          >
            {Math.round(visiblePoints)}pts
          </span>
        )}
      </div>
      ...
      {areTracesVisible && visibleTraces.length > 0 && (
        <div className="ml-8 mt-1 mb-2 p-2 bg-gray-50 rounded-md text-xs">
          {visibleTraces.map((trace, idx) => (
            <div key={`${trace.id}-${idx}`} className="flex items-start gap-2 py-1 hover:bg-gray-100 rounded px-1">
      ...
      {hasChildren && isExpanded && (
        <div>
          {node.children.map((child, idx) => {
            ...
            const childVisible = calculateVisiblePoints(child, currentDate) > 0;
...
const CartographyViewer = () => {
  ...
  useEffect(() => {
    if (isPlaying && data) {
      ...
      animationRef.current = setInterval(animate, 100); // Update every 100ms
...
<CARTO EXAMID="Portfolio-2024" DATE="27 Décembre 2024" TEACHER="Système Adaptatif" LEARNER="Marie Dubois" XML_LANG="FR">
  ...
  const renderSectorsRecursive = (nodes, parentAngleStart, parentAngleEnd, depth, maxDepth, parentColor) => {
    ...
    // Calculate total visible points for proportional sizing
    const totalValue = nodes.reduce((sum, node) => {
      return sum + calculateVisiblePoints(node, currentDate);
    }, 0);
  ...
  const displayNodePoints = displayNode ? calculateVisiblePoints(displayNode, currentDate) : 0;
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-green-600">{data.metadata.progressionTrimestre}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Clock className="text-blue-500" size={20} />
                  <div>
                    <p className="text-xs text-gray-600">Heures visibles</p>
                    <p className="text-lg font-bold">
                      {Math.round(calculateVisiblePoints(data.root, currentDate) / 10)}h
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-gray-600">Activités visibles</p>
                    <p className="text-lg font-bold">
