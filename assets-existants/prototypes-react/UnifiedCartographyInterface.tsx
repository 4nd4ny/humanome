...
import {
    ...
    ExternalLink, ChevronDown, Link as LinkIcon,
    RotateCcw
} from "lucide-react";

/**
 * Unified Cartography Interface
 * 
 * Merges:
 * - Layout & Logic from unified-cartography.tsx
 * - Superior Radial Visualization (CircularSector) from cartography-viewer.tsx
 */
...
// -------------------- Circular Sector (from cartography-viewer) --------------------

interface CircularSectorProps {
    sector: NodeT;
    centerX: number;
    centerY: number;
    innerRadius: number;
...
const CircularSector: React.FC<CircularSectorProps> = ({
    sector,
    centerX,
    centerY,
    innerRadius,
    ...
    const effectiveOuterRadius = useMemo(() => {
        if (sector.isLeaf && sector.niveau) {
            ...
            const factor = niveaux[sector.niveau]?.radiusFactor || 0.4;
...
const UnifiedCartographyInterface: React.FC<Props> = ({ xmlText }) => {
    ...
    const [maskListBefore, setMaskListBefore] = useState<boolean>(true);
        ...
        for (let w = 0; w < weeks; w++) {
            ...
            for (let d = 0; d < 7; d++) {
                ...
                days.push({
                    ...
                    isFuture: iter > pivotDate, // mask following days
    ...
    // Radial sectors (Improved version from cartography-viewer)
                ...
                <g>
                    {sectors.map((s, i) => (
                        <CircularSector
                            ...
                            endAngle={s.end}
                            color={s.color}
                            isSelected={focusNodeId === s.node.id}
                            isHovered={hoveredNodeId === s.node.id}
                            onClick={() => setFocusNodeId(s.node.id)}
                            onMouseEnter={() => setHoveredNodeId(s.node.id)}
                            onMouseLeave={() => setHoveredNodeId(null)}
                            maxOuterRadius={baseInner + ringWidth * 3} // Approx max
                            isVisible={true}
                        />
                    ))}
...
export default UnifiedCartographyInterface;
