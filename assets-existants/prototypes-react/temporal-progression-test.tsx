...
const TemporalProgressionViewer = () => {
  ...
  const calendarGrid = useMemo(() => {
    ...
    return grid;
  }, [selectedYear, activityMap, currentDate]);
  ...
  const stats = useMemo(() => {
    ...
    const visible = xmlData.filter(t => new Date(t.date) <= currentDate);
    return {
  ...
  const handleActivityClick = (trace) => {
    console.log('Activité cliquée :', trace);
  };
        ...
        <div className="min-w-max">
          ...
          <div className="flex gap-[1px]" style={{ marginTop: '20px' }}>
            ...
            <div className="flex flex-col gap-[1px] mr-2 text-[10px] text-gray-600">
              {dayLabels.map((day, i) => (
                <div key={i} className="h-[11px] w-6 text-right pr-1">
                  {day}
                </div>
              ))}
            </div>
              ...
              {calendarGrid.map((week, weekIndex) => (
                <div key={weekIndex} className="flex flex-col gap-[1px]">
                  {week.map((day, dayIndex) => {
                    ...
                    if (!day.isInYear) {
                      // Cases hors de l'année - transparentes
          ...
          <div className="flex items-center gap-2 mt-4 text-xs text-gray-600">
            ...
            <div className="flex gap-[2px]">
              <div className="w-[11px] h-[11px] rounded-sm" style={{ backgroundColor: '#ebedf0', border: '1px solid rgba(27, 31, 35, 0.06)' }} />
              <div className="w-[11px] h-[11px] rounded-sm" style={{ backgroundColor: '#9be9a8', border: '1px solid rgba(27, 31, 35, 0.06)' }} />
              <div className="w-[11px] h-[11px] rounded-sm" style={{ backgroundColor: '#40c463', border: '1px solid rgba(27, 31, 35, 0.06)' }} />
              <div className="w-[11px] h-[11px] rounded-sm" style={{ backgroundColor: '#30a14e', border: '1px solid rgba(27, 31, 35, 0.06)' }} />
              <div className="w-[11px] h-[11px] rounded-sm" style={{ backgroundColor: '#216e39', border: '1px solid rgba(27, 31, 35, 0.06)' }} />
            </div>
            <span>Plus</span>
          </div>
        </div>
      ...
      {xmlData && (
        <div className="mt-6 border-t pt-4">
          <h3 className="text-lg font-semibold mb-3">
            Activités jusqu'au {currentDate.toLocaleDateString('fr-FR')}
          </h3>
